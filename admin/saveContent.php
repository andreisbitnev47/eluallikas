<?php
	...
	if ($_SERVER[REQUEST_METHOD]=='POST'){
		...
		switch ($tag){
			case "h1": $finInput = $a.'<'.$tag.' style=\'text-align:'.$align.'; font-size:'.$fontSize.'\'>'.$input.'</'.$tag.'>'.$aClose; break;
			case "p": $finInput = $a.'<'.$tag.' style=\'font-size:'.$fontSize.'\'>'.$input.'</'.$tag.'>'.$aClose; break;
			case "img": $finInput = $a.$div.'<'.$tag.' width=\''.$width.'\' height=\''.$height.'\' style=\''.$float.'\' src=\''.$src.'\'>'.$divClose.$aClose; break;
			case "vid": $finInput = '<div class=\'vid\' style=\'width:'.$videoWidth.'px;\'><h3>'.$title.'</h3>'.$videoHref.'<p>'.$descr.'</p></div>';break;
			case "hr": $finInput = '<hr style=\'clear:both;'.$hrWidth.'\'>';break;
			case "album": $oneAlbum = $albums->getOneDir($albumDir);
						$title = $oneAlbum[0]['title'];
						$src = $oneAlbum[0]['src'];
		...
		$res=$content->saveContent($id, $finInput, $pageName);
